...
void SubSymbolGenerator::z_decompose_post(
  stream<ap_uint <Z_META_STREAM_SIZE> > &z_stream_with_meta,
  stream<subsymb_t> &z_decomposed){
  #pragma HLS INTERFACE ap_ctrl_none port=return
  #pragma HLS PIPELINE style=flp 
...
void SubSymbolGenerator::serialize_symbols(
  stream<ap_uint <Y_SIZE+P_SIZE> > &y_stream,
  stream<subsymb_t > &z_decomposed,
  stream<subsymb_t> &symbol_stream){
  // #pragma HLS INTERFACE ap_ctrl_none port=return
...
void SubSymbolGenerator::transform(
  stream<coder_interf_t> &in,
  stream<subsymb_t> &symbol_stream
  ){
...
void subsymbol_gen_double_lane(
  ...
  static SubSymbolGenerator generator_0,generator_1;
  #pragma HLS reset variable=generator_0
  #pragma HLS reset variable=generator_1
  
  generator_0.transform(in_0,symbol_stream_0);
  generator_1.transform(in_1,symbol_stream_1);
